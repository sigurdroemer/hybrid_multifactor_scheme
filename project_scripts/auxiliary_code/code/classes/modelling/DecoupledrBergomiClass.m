classdef DecoupledrBergomiClass < PricingModelClass & handle
%
...
%   where 
%
%     V(t) = c + zeta(t)*exp(eta*(theta*X_1(t) + (1-theta)*X_2(t)) - f(t))
%
%   with
%
%   X_1(t) = int_0^t K_1(t-s)dW2(t)
%
%   X_2(t) = int_0^t K_2(t-s)dW3(t)
%
%   f(t) = (1/2)*Var(eta*(theta*X_1(t) + (1-theta)*X_2(t)))
%
%        = (1/2)*(eta^2)*(   theta^2*int_0^t K_1(s)^2 ds
%
%                          + (1-theta)^2*rho_23^2*int_0^t K_2(s)^2 ds
%
%                          + 2*theta*(1-theta)*rho_23*int_0^t K_1(s)K_2(s) ds
%
%                          + (1-theta)^2*(1-rho_23)*int_0^t K_2(s)^2 ds
%
%                         )
%
%   with zeta, K_1, K_2 being deterministic functions (K's completely monotone) and 
%   where c, eta > 0, 0 <= theta <= 1 and W = (W1,W2,W3) have a general correlation
%   structure as specified by parameters rho_12, rho_13, rho_23. 
%
%   Remark: See pdf note for a derivation of the explicit expression for f(t).
% 
% -----------------------------------------------------------------------------------------------
%  Properties  
% -----------------------------------------------------------------------------------------------
...
%   K_1:      [1x1 KernelFunctionClass] Kernel of X_1.
%   K_2:      [1x1 KernelFunctionClass] Kernel of X_2.
%   zeta:     [1x1 CurveClass]          Curve to control the mean term structure of V(t). 
%                                       The forward variance curve is related as 
%                                       xi(t) := zeta(t) + c.
%
...
methods
   function obj = DecoupledrBergomiClass(varargin)
    % 	
    %   Constructor.
    %
   ...
   % ---------------------------------------------------------------------------------------------   
       ...
       if condMC_S
           error(['DecoupledrBergomiClass:GetPricesSub: Conditional Monte Carlo is not ',...
       ...
       if ~(strcmpi(cvS,'none') || strcmpi(cvS,'asset_price'))
           error(['DecoupledrBergomiClass:GetPricesSub: obj.price_estimation.S.control_variate ',...
                  'only supports options ''none'' and ''asset_price''.']);
       end
       if ~(strcmpi(cvV,'none') || strcmpi(cvV,'VIX2'))
           error(['DecoupledrBergomiClass:GetPricesSub: obj.price_estimation.VIX.control_variate ',...
   ...
   % ---------------------------------------------------------------------------------------------
       ...
       if ~strcmpi(scheme,'hybrid_multifactor')
          error(['DecoupledrBergomiClass:GetNumbersForSim: Scheme ', scheme, ' is not supported.']);
       end
       ...
       if anti
           ...
           if rem(N,2) ~= 0
               error(['DecoupledrBergomiClass:GenNumbersForSim: When ',...
   ...
   % ---------------------------------------------------------------------------------------------
       ...
       if ~isa(settings,'MonteCarloPricerSettingsClass')
           error(['DecoupledrBergomiClass:GenNumbersForMC: Pricer settings must ', ...
   ...
   % ---------------------------------------------------------------------------------------------
       ...
       if rho12^2 + rho13^2 + rho23^2 - 2*rho12*rho13*rho23 - eps > 1
           error('DecoupledrBergomiClass:Simulate: Invalid correlation matrix.');
       end
       
       % Check output variables are valid:
       expectedOutVars = {'S','V','VIX2','U1','U2'};
       if sum(ismember(variables,expectedOutVars)) ~= size(variables,2)
           error(['DecoupledrBergomiClass:Simulate: Some of the chosen output variables ',...
                  'are not supported.']);
       end
       
       % Check for duplicates:
       if size(unique(variables),2) ~= size(variables,2)
           error(['DecoupledrBergomiClass:Simulate: Values in ''variables'' parameter ',...
                  'must be unique.']);
       end
       
       if size(variables,1) ~= size(timepoints,1) || size(variables,2) ~= size(timepoints,2)
           error(['DecoupledrBergomiClass:Simulation: The inputs ''variables'' and ',...
                  '''timepoints'' must have the same dimensions']);
       end
       
       if antithetic && rem(N,2) ~= 0
           % Check divisibility by 2
           error(['DecoupledrBergomiClass:Simulate: When ',...
       ...
       for i=1:size(timepoints,2)
           if size(timepoints{i},1) > 1
               error(['DecoupledrBergomiClass:Simulate: The elements of ''timepoints'' ',...
                      'must all be row vectors.']);
           end
           if ~issorted(timepoints{i},'ascend')
               error(['DecoupledrBergomiClass:Simulate: Each element of ''timepoints'' ',...
       ...
       elseif strcmpi(n_is,'total')
            ...
            error('DecoupledrBergomiClass:Simulate: Invalid value of input parameter ''n_is''.');
       end
       n_per_unit = 1/dt;
       ...
       if ~isempty(idxVIX2) && isempty(nI_VIX2)
           error(['DecoupledrBergomiClass:Simulate: When simulating ''VIX2'' we require ',...
       ...
       if kappa ~= 1 && ~isempty(idxVIX2)
           error(['DecoupledrBergomiClass:Simulate: We currently require ',...
       ...
       if Npts > kernelApproxSettings.n_cap
           ...
           warning(['DecoupledrBergomiClass:Simulate: Number of sampling points for ',...
       ...
       [c2_sqr,gamm2_sqr] = ApproximateKernel('BM2005','K',@(t)(K2.Eval(t).^2),'T',T_approx_K,...
                                    ...
                                    'epsilon',kernelApproxSettings.epsilon,...
                                    'error_measure',kernelApproxSettings.error_measure);
       [c12,gamm12] = ApproximateKernel('BM2005','K',@(t)(K1.Eval(t).*K2.Eval(t)),'T',T_approx_K,...
                                    'delta',max(dt,dt*kappa),'N',Npts,...
                                    'epsilon',kernelApproxSettings.epsilon,...
                                    'error_measure',kernelApproxSettings.error_measure);                                
      ...
      if ~any(ismember(variables,{'U1','U2','VIX2'}))
       ...
       switch simSettings.scheme
           case 'hybrid_multifactor'
               ...
               [Y1,U1] = HybridMultifactorScheme(N,nStepsTotalSim/T,[],0,...
                                     0,eta*theta,gamm1,c1,kappa,'K',K1,'returnU',returnU,...
                                     'positive',false,'tX',tV_sim,...
                                     'tU',tU_sim,'returndW',false,...
                                     'precision',prec,'explicit',simSettings.explicit,...
                                     'W_bold',W_bold{2});
               [Y2,U2] = HybridMultifactorScheme(N,nStepsTotalSim/T,[],0,...
                                          0,eta*(1-theta),gamm2,c2,kappa,...
           ...
           otherwise
               error('DecoupledrBergomiClass:Simulate: Scheme is not supported.');
       end
       ...
       if ~isempty(idxV) || ~isempty(idxS) || ~isempty(idxVIX2)
           ...
           % Compute integrated K's:           
           K1_sqr_int_tmp = [0;cumsum(K1.Integrate(t_lb,t_ub,2,max(dt,dt*kappa),c1_sqr',gamm1_sqr'))]';
           K2_sqr_int_tmp = [0;cumsum(K2.Integrate(t_lb,t_ub,2,max(dt,dt*kappa),c2_sqr',gamm2_sqr'))]'; 
           K1K2_int_tmp = [0;cumsum(K1K2.Integrate(t_lb,t_ub,1,max(dt,dt*kappa),c12',gamm12'))]';                                                       
               
           idxt = LocateTimepointsInDiscretisation([0;t_ub]',tTruncV);
           K1_sqr_int = K1_sqr_int_tmp(idxt);
           K2_sqr_int = K2_sqr_int_tmp(idxt);
           K1K2_int = K1K2_int_tmp(idxt);
           
           % Compute f-values:
           ft = (1/2)*(eta^2)*(theta^2*K1_sqr_int + (1-theta)^2*rho23^2*K2_sqr_int ...
                + 2*theta*(1-theta)*rho23*K1K2_int + (1-theta)^2*(1-rho23)*K2_sqr_int);
           
           % Compute V:
           V = obj.c + zeta(tTruncV).*exp(Y1.values + Y2.values - ft);
       ...
       if ~isempty(idxVIX2)
           % Compute time points:
            ...
            for i=1:size(paths.VIX2.t,2)
                % Compute integrated K's:
                K1_sqr_int = K1.Integrate(tauPtsHighAdd',paths.VIX2.t(i) + tauPtsHighAdd',...
                                            2,max(dt,dt*kappa),c1_sqr',gamm1_sqr').';
                K2_sqr_int = K2.Integrate(tauPtsHighAdd',paths.VIX2.t(i) + tauPtsHighAdd',...
                                            2,max(dt,dt*kappa),c2_sqr',gamm2_sqr').';                                        
                K1K2_int = K1K2.Integrate(tauPtsHighAdd',paths.VIX2.t(i) + tauPtsHighAdd',...
                                            1,max(dt,dt*kappa),c12',gamm12').';     
                                        
                % Compute f(t,u):
                ftu = (1/2)*(eta^2)*(theta^2*K1_sqr_int + (1-theta)^2*rho23^2*K2_sqr_int ...
                                   + 2*theta*(1-theta)*rho23*K1K2_int ...
                                   + (1-theta)^2*(1-rho23)*K2_sqr_int);                            
                               
                % Compute integration values:
                intVals = U1.values(:,:,idxtU(i))*dummy1 + U2.values(:,:,idxtU(i))*dummy2 - ftu;
                
                % Compute forward variances at longer maturities:
                zetaVals = zeta(paths.VIX2.t(i) + tauPtsHigh);
                xiVals(:,nPtsLow+1:end) = obj.c + zetaVals.*exp(intVals(:,2:end));
                
                % Compute xi(dt) value:
                xi_dt = obj.c + zeta(paths.VIX2.t(i)+dt).*exp(intVals(:,1));
