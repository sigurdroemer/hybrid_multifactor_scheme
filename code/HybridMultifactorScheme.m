function [Xpaths,Upaths,dW] = HybridMultifactorScheme(N,n,T,x0,b,sigma,gamm,c,kappa,varargin)
% Description: Simulates the stochastic Volterra equation
%
% X(t) = x0 + int_0^t K(t-s)b(s)ds + int_0^t K(t-s)sigma(s)dW(s)
%
...
%               and then use the hybrid-exponential scheme to first simulate the processes on the 
...
if isempty(W_bold)
    ...
    if isempty(SIGMA)
        if isempty(K) && kappa > 0
            error(['HybridMultifactorScheme: When the covariance matrix SIGMA is not inputted, ',...
        ...
        if size(SIGMA,1) ~= kappa + 1 || size(SIGMA,2) ~= kappa + 1
            error(['HybridMultifactorScheme: The covariance matrix SIGMA must be of the size ',...
                   '[(kappa + 1) x (kappa + 1)].']);
        end
    ...
    try
        ...
        warning(['HybridMultifactorScheme: The covariance matrix was not positive ',...
    ...
    if size(W_bold,1) ~= N || size(W_bold,2) ~= M - 1 || size(W_bold,3) ~= kappa + 1
        error('HybridMultifactorScheme: The pre-simulated W_bold-matrix has the wrong dimensions.');
    end
...
if kappa > 0
    if isempty(w)
        ...
        w = zeros(1,kappa,precision);
        for k=1:kappa
            w(k) = integral(K,(k-1)/n,k/n);
        end
    else
